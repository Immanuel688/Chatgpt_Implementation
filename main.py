...
# HERE WE GET THE THREAD ID FOR THE CONVERSATION 
...
def chat():
  ...
  while True:
    ...
    sleep(1)  
    # sleep holds the loop execution for 1 second so that it doesn't 
    # burn all api requests in milliseconds
  ...
  if lead.find('{') != -1: #.find() is a string search function that returns the index (position) where a substring is found. If the substring is not found, it returns -1.
    import json
